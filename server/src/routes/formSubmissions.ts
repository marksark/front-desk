...
import { listFormSubmissions, appendFormSubmission } from "../lib/formSubmissionsStore";
...
router.post("/:tenantId", async (req, res) => {
  ...
  const entry = await appendFormSubmission(tenantId, body.formData);
  if (entry === null) {
    return res.status(400).json({ error: "Invalid form data." });
  }

  return res.status(201).json({ submission: entry });
});
