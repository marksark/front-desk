...
function App() {
  ...
  const isFormIntakeRoute = routePath === "/intake";
  const formIntakeTenant = useMemo(() => {
    const params = new URLSearchParams(window.location.search);
    return params.get("tenant")?.trim() ?? null;
  }, [routePath]);
  const persistentNavTarget = isFormBuilderRoute
    ...
    : isOperatorRoute
      ...
      : isFormIntakeRoute
        ? { href: "/operator", label: "Back to staff" }
  ...
  const loadFormSubmissions = useCallback(async () => {
    ...
    try {
      ...
      const payload = (await response.json()) as { submissions: FormSubmission[] };
  ...
  if (isFormIntakeRoute) {
    if (!formIntakeTenant) {
      return (
        <div className="route-fallback">
          <p>Choose a school to open the intake form.</p>
          <p>
            Example: add <code>?tenant=sunshine-academy</code> to the URL.
          </p>
        </div>
      );
    }
    return (
      <>
        <FormIntakePage tenantId={formIntakeTenant} title="Intake form" />
    ...
    return (
      <div className="route-fallback">
        ...
        <div className="route-links">
          ...
          <a href="/intake?tenant=sunshine-academy">Go to intake form (example tenant)</a>
